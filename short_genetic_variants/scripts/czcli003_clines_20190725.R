...
if (zone=="CZA_left" | zone=="CZA_right"){
  ...
  snail <- read.csv(paste("../", zone, "_spatial_LCP_201901.csv", sep=""))
  snail <- merge(snail,read.csv("../CZA_dissections_20150506.csv"),by="snail_ID", all=T)
}

if (zone=="CZB_left" | zone=="CZB_right"){
  rt <- 20.45
  snail <- read.csv(paste("../", zone, "_spatial_LCP_201901.csv", sep=""))
  snail <- merge(snail,read.csv("../CZB_dissections_20150601.csv"),by="snail_ID", all=T)
}

if (zone=="CZD_left" | zone=="CZD_right"){
  rt <- 42.50
  snail <- read.csv(paste("../", zone, "_spatial_LCP_201901.csv", sep=""))
  snail <- merge(snail,read.csv("../CZD_dissections_20150601.csv"),by="snail_ID", all=T)
}
